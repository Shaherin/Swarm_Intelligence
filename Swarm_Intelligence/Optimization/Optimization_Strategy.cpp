...
void Optimization_Strategy::loadData(const string filename)
{
    ...
    if(file.is_open())
    {
        ...
        while(getline(file, line))
        {
            ...
            {
                current_set++;
                //Process_Sets[current_set] = Process_Manager();
                Process_Sets.push_back(Process_Manager());
            }
            ...
            if(regex_match(line, regex_matcher, regex_process_triple))
            {
                ...
                Process_Sets[current_set].addProcess( move(Process(time_duration(processing_time), earliness_penalty, tardiness_penalty)) );
            }
        }
    }
};
